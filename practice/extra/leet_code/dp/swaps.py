...
# swap(i) - minimalna ilosc zamian do itego indeksu zamieniajac ostatni indeks
# no_swap(i) - minimalna ilosc zamian do itego indeksu nie zamieniajac ostatniego indeksu

def min_swaps(arr1, arr2):

    n = len(arr1)

    no_swap = [float("inf")] * n       # nie zamieniamy pozycji na itym indeksie
    swap = [float("inf")] * n          # zamieniamy pozycje na itym indeksie
    ...
    for i in range(1, n):
        ...
        str_i = arr1[i-1] < arr1[i] and arr2[i-1] < arr2[i]
        str_i_x = arr1[i-1] < arr2[i] and arr2[i-1] < arr1[i]
        ...
        if str_i:
            ...
            swap[i] = min(swap[i], swap[i-1] + 1)           # jesli wczesniej zmienilismy i teraz chcemy zmienic to bierzemy poprzednia wartosc
            no_swap[i] = min(no_swap[i], no_swap[i-1])      # jesli nie zmienilismy pozycji to nic nie robimy
